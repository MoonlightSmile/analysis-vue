...
import type { Spring } from '../utils/WaveTank'
import { WaveTank } from '../utils/WaveTank'
const SVG_WIDTH = 100
...
const state = reactive({
  ...
  dropy: 60,
  width: SVG_WIDTH,
  springs: waveTank.springs as Spring[],
})
...
const points = $computed(() => [
  [0, 100],
  [0, 0],
  ...state.springs.map((x, i) => [i * grid, x.p]),
  [state.width, 0],
  [state.width, 100],
])
const springsPath = $computed(() => `${points.map(x => x.join(',')).join(' ')}`)
const juice = $computed(() => `M18 ${63 + state.counter} C15 ${63 + state.counter} 16 ${63 + state.counter
  } 12 61L9 56C2 33 62 -3 80 12C103 27 44 56 29 58C27 58 25 59 24 61C20 ${63 + state.counter
  } 21 ${63 + state.counter} 18 ${63 + state.counter}Z`)
...
function updateJuice(timestamp: number) {
  ...
  if (saw < 0.6) {
    state.counter = (easeInCirc(saw) * amp)
    state.dropy = -100
  }
  else {
    state.counter = (easeInCirc(1 - saw) * amp * 0.1)
    state.dropy = (70 + Math.pow(saw - 0.6, 2) * 10000)
  }
}
...
function update(timestamp: number) {
  ...
  state.springs = ([...waveTank.springs])
  ...
  requestIdRef.value = requestAnimationFrame(update)
}
...
function drop() {
  const dropPosition = Math.round(
    ((state.width / 2 - 30) / state.width) * 100,
  )
...
<template>
  <div class="w-full flex justify-center items-center flex-col bg-green-300">
    <svg
      width="100" height="160" viewBox="0 0 100 160" fill="none" xmlns="http://www.w3.org/2000/svg" class="mt-28"
      role="img" aria-label="Fresh logo"
    >
      <circle cx="18" :cy="state.dropy" r="4" fill="white" />
      <!-- <path
        ...
        fill="#0A140C"
      /> -->
      <path d="M84 16c13 27 1 52-7 59 0 4-9 9-12 7-12 5-38-2-53-21-6-7 1-21 21-36 13-10 33-17 51-9Z" fill="#FFD80B" />
    ...
    <svg width="100%" height="100px" viewBox="0 0 100 100" preserveAspectRatio="none" class="-mt-5">
      <polygon :points="springsPath" fill="white" transform="translate(0, 50)" />
    </svg>
  </div>
</template>
